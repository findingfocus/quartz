// Site-owned fork of quartz-community/graph renderer.
// Diff vs upstream: vendored path utils below (no imports, this file ships
// raw to the browser) plus the folder-form centre fix after the data map.
// It will NOT track upstream graph updates; re-diff on plugin upgrades.
...
;(function () {
  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      // Folder homepages arrive slashless ("11-beyond") while map keys
      // are folder-form ("11-beyond/"): resolve to the form that exists
      // so the centre node gets its title and its neighbours
      if (!data.has(slug) && data.has(slug + "/")) slug = slug + "/"
