---
title: Start Here
description: New to game dev? Pick your path and ship your first playable in under an hour.
tags:
  - hub
  - beginner
---

**Solo game dev is hard. But you can do it.** This hub is the pipeline I wish I had: free tools, tiny concepts, and a straight line from zero to a game you can share.

> [!QUOTE]
> "This second we can turn the tables on Resistance. This second, we can sit down and do our work." — Steven Pressfield, *The War of Art*

## Pick your path

| Path | You are... | Do this | Time |
| ---- | ---------- | ------- | ---- |
| **A. Never coded** | "I don't know anything about code" | [[02-Setup/index\|Setup]] → [[03-Lua-Essentials/index\|Lua Essentials]] → [[05-First-Game/index\|First Game]] | ~60 min |
| **B. Coded, new to games** | "I know some programming" | [[02-Setup/index\|Setup]] → [[04-Love2D-Basics/index\|Love2D Basics]] → [[05-First-Game/index\|First Game]] | ~30 min |
| **C. Just want to tinker** | "Give me code now" | [[06-Starter-Template/index\|Starter Template]] → [[10-Demos/index\|Play a demo]] → [[07-Cookbook/index\|Cookbook]] | ~10 min |

## The 4-step loop

1. **Setup** — install Love2D, open a folder, run a window. [[02-Setup/index\|Setup guide]]
2. **Make** — follow one guided game. [[05-First-Game/index\|Your first game]]
3. **Steal** — grab the template and cookbook recipes. [[06-Starter-Template/index|Template]] · [[07-Cookbook/index|Cookbook]]
4. **Share** — put it on web + desktop. [[09-Distribution/index\|Distribution]]

## What you need

- A computer (Windows, Mac, or Linux — all work)
- [Love2D](https://love2d.org/) (free, ~10MB)
- Any text editor ([VS Code](https://code.visualstudio.com/) recommended for beginners)
- No money, no game engine account, no experience

> [!TIP]
> Stuck? Every page has a **Try it** step and a **When it breaks** box. Read the break box first — that's where the learning lives.

## Next

> [!tip] Next step
> Go to [[02-Setup/index|Setup]] and get a window on screen. That's your first win.
